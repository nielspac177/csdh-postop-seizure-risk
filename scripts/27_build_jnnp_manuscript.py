...
def build_main():
    ...
    add_runs(doc, [("Results. ", {"bold": True}),
        ("Firth penalized logistic regression was the candidate model, fit "
         ...
         "whereas universal AED was preferable only under optimistic, "
         "imported efficacy assumptions. Population value of perfect "
    ...
    register_figure("Figure 2", FIG / "F2_calibration_dca.png",
                ...
                "model. A, Reliability diagram (observed event rate versus mean "
                "predicted risk per quantile bin, with Wilson 95% confidence "
                "intervals) for the deployed BIDMC postoperative-B (Firth) "
                "model and the external eICU Set C model; the marginal rug shows "
                "the density of predicted risk. The deployed model is well "
                "calibrated in the mean (mean predicted risk equals the 7% base "
                "rate) but under-dispersed, so its predictions are compressed "
                "into a narrow low range near the base rate rather than "
                "extending up the diagonal. B, Decision-curve net benefit "
